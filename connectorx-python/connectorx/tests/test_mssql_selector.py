...
@pytest.mark.parametrize("output", ["pandas", "arrow", "arrow_stream", "polars"])
@pytest.mark.parametrize("shape", ["values", "nulls", "empty"])
def test_live_bridge_fixed_width_values_and_metadata(selector_uri, output, shape):
    run_child(
        """
import os
import connectorx as cx
...
output = os.environ["CX_SELECTOR_OUTPUT"]
shape = os.environ["CX_SELECTOR_SHAPE"]
query = ("SELECT CAST('xy' AS char(4)) AS c, CAST(CHAR(1) + CHAR(2) AS binary(4)) AS b, "
         "CAST(-2.5 AS smallmoney) AS m")
if shape == "nulls":
    query = ("SELECT CAST(NULL AS char(4)) AS c, CAST(NULL AS binary(4)) AS b, "
             "CAST(NULL AS smallmoney) AS m")
elif shape == "empty":
    query += " WHERE 1=0"
assert list(cx.get_meta(uri, query).columns) == ["c", "b", "m"]
result = cx.read_sql(uri, query, return_type=output, batch_size=1)
if output == "arrow_stream":
    result = result.read_all()
if output == "polars":
    result = result.to_arrow()
if output == "pandas":
    assert list(result.columns) == ["c", "b", "m"]
    if shape == "nulls":
        assert len(result) == 1
        assert result.isna().all().all()
    values = result.to_dict(orient="list")
else:
    assert result.column_names == ["c", "b", "m"]
    values = result.to_pydict()
    if shape == "nulls":
        assert values == {"c": [None], "b": [None], "m": [None]}
if shape == "values":
    assert values == {"c": ["xy  "], "b": [bytes([1, 2, 0, 0])], "m": [-2.5]}, values
elif shape == "empty":
    assert values == {"c": [], "b": [], "m": []}, values
""",
        "mssql-tds",
        CX_SELECTOR_URI=selector_uri,
        CX_SELECTOR_OUTPUT=output,
